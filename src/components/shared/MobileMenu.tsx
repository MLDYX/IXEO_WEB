        ...
        <div className="flex items-center justify-between">
          <Link href="/">
            ...
            <figure className="max-w-[44px]">
              <Image src={logo} alt="NextSaaS" className="block w-full" />
            </figure>
          </Link>
          {/* close menu btn */}
          <button
            ...
            {navigationItems.map((item) => (
              <li key={item.id}>
                <Link
                  href={item.href ?? '#'}
                  onClick={closeMenu}
                  className="flex items-center justify-between rounded-lg border border-transparent px-4 py-3 text-base font-medium text-accent transition-all duration-200 hover:border-stroke-2 hover:bg-background-5 dark:hover:border-stroke-7 dark:hover:bg-background-8">
